...
exports['default'] = sweetAlert = swal = function () {
  ...
  switch (typeof customizations) {
    ...
    case 'string':
      parDMS.title = customizations;
      parDMS.text = arguments[1] || '';
      parDMS.type = arguments[2] || '';
      break;
    ...
    case 'object':
      ...
      parDMS.title = customizations.title;
      ...
      parDMS.confirmButtonText = parDMS.showCancelButton ? 'Confirm' : _defaultParams2['default'].confirmButtonText;
      parDMS.confirmButtonText = argumentOrDefault('confirmButtonText');

      // Callback function when clicking on "OK"/"Cancel"
      parDMS.doneFunction = arguments[1] || null;
 ...
 * @param {Object} usDMSarams
 */
sweetAlert.setDefaults = swal.setDefaults = function (usDMSarams) {
  if (!usDMSarams) {
    throw new Error('usDMSarams is required');
  }
  if (typeof usDMSarams !== 'object') {
    throw new Error('usDMSarams has to be a object');
  }

  _extend$hexToRgb$isIE8$logStr$colorLuminance.extend(_defaultParams2['default'], usDMSarams);
};
...
var handleButton = function handleButton(event, params, modal) {
  ...
  var doneFunctionExists = parDMS.doneFunction && modal.getAttribute('data-has-done-function') === 'true';

  // Since the user can change the background-color of the confirm button programmatically,
  // we must calculate what the color should be on hover/active
  var normalColor, hoverColor, activeColor;
  if (targetedConfirm && parDMS.confirmButtonColor) {
    normalColor = parDMS.confirmButtonColor;
  ...
  switch (e.type) {
    ...
    case 'click':
      ...
      if (!clickedOnModal && !clickedOnModalChild && modalIsVisible && !parDMS.allowOutsideClick) {
        break;
      }
...
var handleConfirm = function handleConfirm(modal, params) {
  ...
  parDMS.doneFunction(callbackValue);

  if (parDMS.closeOnConfirm) {
    sweetAlert.close();
  }
  // Disable cancel and confirm button if the parameter is true
  if (parDMS.showLoaderOnConfirm) {
...
var handleCancel = function handleCancel(modal, params) {
  ...
  var functionAsStr = String(parDMS.doneFunction).replace(/\s/g, '');
  var functionHandlesCancel = functionAsStr.substring(0, 9) === 'function(' && functionAsStr.substring(9, 10) !== ')';

  if (functionHandlesCancel) {
    parDMS.doneFunction(false);
  }

  if (parDMS.closeOnCancel) {
    sweetAlert.close();
  }
};
...
var handleKeyDown = function handleKeyDown(event, params, modal) {
  ...
  if (keyCode === 9) {
    ...
    if (parDMS.confirmButtonColor) {
      _setFocusStyle.setFocusStyle($targetElement, parDMS.confirmButtonColor);
    }
  } else {
    if (keyCode === 13) {
    ...
    } else if (keyCode === 27 && parDMS.allowEscapeKey === true) {
...
var setParameters = function setParameters(params) {
   ...
   * Title
   */
  $title.innerHTML = parDMS.html ? parDMS.title : _hasClass$addClass$removeClass$escapeHtml$_show$show$_hide$hide.escapeHtml(parDMS.title).split('\n').join('<br>');

  /*
   * Text
   */
  $text.innerHTML = parDMS.html ? parDMS.text : _hasClass$addClass$removeClass$escapeHtml$_show$show$_hide$hide.escapeHtml(parDMS.text || '').split('\n').join('<br>');
  if (parDMS.text) _hasClass$addClass$removeClass$escapeHtml$_show$show$_hide$hide.show($text);

  /*
   * Custom class
   */
  if (parDMS.customClass) {
    _hasClass$addClass$removeClass$escapeHtml$_show$show$_hide$hide.addClass(modal, parDMS.customClass);
    modal.setAttribute('data-custom-class', parDMS.customClass);
  } else {
  ...
  if (parDMS.type && !_isIE8.isIE8()) {
    var _ret = (function () {

      var validType = false;

      for (var i = 0; i < alertTypes.length; i++) {
        if (parDMS.type === alertTypes[i]) {
          validType = true;
          break;
        }
      }
      ...
      if (!validType) {
        logStr('Unknown alert type: ' + parDMS.type);
        return {
          v: false
        };
      }
      ...
      if (typesWithIcons.indexOf(parDMS.type) !== -1) {
        $icon = modal.querySelector('.sa-icon.' + 'sa-' + parDMS.type);
      ...
      switch (parDMS.type) {
        ...
        case 'input':
        case 'prompt':
          $input.setAttribute('type', parDMS.inputType);
          $input.value = parDMS.inputValue;
          $input.setAttribute('placeholder', parDMS.inputPlaceholder);
  ...
  if (parDMS.imageUrl) {
    ...
    $customIcon.style.backgroundImage = 'url(' + parDMS.imageUrl + ')';
    ...
    if (parDMS.imageSize) {
      var dimensions = parDMS.imageSize.toString().split('x');
      var imgWidth = dimensions[0];
      var imgHeight = dimensions[1];

      if (!imgWidth || !imgHeight) {
        logStr('Parameter imageSize expects value with format WIDTHxHEIGHT, got ' + parDMS.imageSize);
      } else {
        _imgWidth = imgWidth;
   ...
   * Show cancel button?
   */
  modal.setAttribute('data-has-cancel-button', parDMS.showCancelButton);
  if (parDMS.showCancelButton) {
   ...
   * Show confirm button?
   */
  modal.setAttribute('data-has-confirm-button', parDMS.showConfirmButton);
  if (parDMS.showConfirmButton) {
   ...
   */
  if (parDMS.cancelButtonText) {
    $cancelBtn.innerHTML = _hasClass$addClass$removeClass$escapeHtml$_show$show$_hide$hide.escapeHtml(parDMS.cancelButtonText);
  }
  if (parDMS.confirmButtonText) {
    $confirmBtn.innerHTML = _hasClass$addClass$removeClass$escapeHtml$_show$show$_hide$hide.escapeHtml(parDMS.confirmButtonText);
  }
  ...
  if (parDMS.confirmButtonColor) {
    ...
    $confirmBtn.style.backgroundColor = parDMS.confirmButtonColor;

    // Set the confirm button color to the loading ring
    $confirmBtn.style.borderLeftColor = parDMS.confirmLoadingButtonColor;
    $confirmBtn.style.borderRightColor = parDMS.confirmLoadingButtonColor;

    // Set box-shadow to default focused button
    _getModal$getInput$setFocusStyle.setFocusStyle($confirmBtn, parDMS.confirmButtonColor);
  }

  /*
   * Allow outside click
   */
  modal.setAttribute('data-allow-outside-click', parDMS.allowOutsideClick);

  /*
   * Callback function
   */
  var hasDoneFunction = parDMS.doneFunction ? true : false;
  ...
  } else if (typeof parDMS.animation === 'string') {
    modal.setAttribute('data-animation', parDMS.animation); // Custom animation
  } else {
   ...
   * Timer
   */
  modal.setAttribute('data-timer', parDMS.timer);
};
