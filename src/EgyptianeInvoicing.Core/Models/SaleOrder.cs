...
{
    public class SaleOrder : AggregateRoot, IAuditableEntity
    {
        ...
        public static Result<SaleOrder> Create(
            Guid sellerId,
            Guid buyerId,
            string? orderNumber,
            List<InvoiceLine> orderLines,
            ...
            double extraDiscountAmount = 0)
        {
            ...
            if (sellerId == Guid.Empty)
                return Result.Failure<SaleOrder>("SaleOrder.Create", "Seller ID is required.");

            if (buyerId == Guid.Empty)
                return Result.Failure<SaleOrder>("SaleOrder.Create", "Buyer ID is required.");

            if (string.IsNullOrEmpty(orderNumber))
                return Result.Failure<SaleOrder>("SaleOrder.Create", "Order number is required.");

            if (orderLines == null || orderLines.Count == 0)
                return Result.Failure<SaleOrder>("SaleOrder.Create", "At least one order line is required.");

            if (!Enum.IsDefined(typeof(Currency), currency))
                return Result.Failure<SaleOrder>("SaleOrder.Create", $"Invalid currency type '{currency}'.");
        ...
        {
            if (orderLine == null)
                return Result.Failure<bool>("SaleOrder.AddOrderLine", "Order line cannot be null.");
        ...
        {
            if (orderLine == null)
                return Result.Failure<bool>("SaleOrder.RemoveOrderLine", "Order line cannot be null.");
            ...
            if (removed)
            {
                ...
                return Result.Failure<bool>("SaleOrder.RemoveOrderLine", "Order line not found.");
        }
        ...
        public Result<InvoiceLine> GetOrderLine(Guid orderLineId)
        {
            ...
            if (orderLine == null)
                return Result.Failure<InvoiceLine>("SaleOrder.GetOrderLine", $"Order line with ID '{orderLineId}' not found.");
        ...
        public Result<InvoiceLine> GetOrderLineByName(string orderLineName)
        {
            ...
            if (orderLine == null)
                return Result.Failure<InvoiceLine>("SaleOrder.GetOrderLineByName", $"Order line with name '{orderLineName}' not found.");
        ...
        {
            if (taxTotal == null)
                return Result.Failure<bool>("SaleOrder.AddTaxTotal", "Tax total cannot be null.");
        ...
        public Result<TaxTotal> GetTaxTotalByCode(string taxCode)
        {
            ...
            if (taxTotal == null)
                return Result.Failure<TaxTotal>("SaleOrder.GetTaxTotalByCode", $"Tax total with code '{taxCode}' not found.");
        ...
        {
            if (taxTotal == null)
                return Result.Failure<bool>("SaleOrder.RemoveTaxTotal", "Tax total cannot be null.");

            var removed = _taxTotals.Remove(taxTotal);
            if (removed)
                return Result.Success(true);
            else
                return Result.Failure<bool>("SaleOrder.RemoveTaxTotal", "Tax total not found.");
        }
    }
}
